...
public class GameBoard : MonoBehaviour
{
    ...
    public int[] Board_TileData = new int[board_size * board_size]; //utwórz reprezentacje jednowymiarowa matrycy
        ...
        for (int z = 0; z < 2; z++)
        {
            ...
            for (int x = 0; x < board_size; x++) //podwojny for - obsluga matrycy 2D
            {
                for (int y = 0; y < board_size; y++) //podwojny for - obsluga matrycy 2D
                {
                    ...
                    while (VerificationFindMatch3(for_x,for_y))
                    {
                        Tile[x, y].currency_type = Mathf.CeilToInt(Random.Range(0, amount_of_currency_types)); // przelosuj typ pola
                        for_x = 0; //wyzeruj = warunek wyjscia z petli while
                        for_y = 0; //wyzeruj = warunek wyjscia z petli while
                    }
                }
            }
        }
...
public class FlippedPieces
{
    ...
    public TilePiece GetTheOtherPiece(TilePiece t)
    {
        if (t == origin)
            return destination;
